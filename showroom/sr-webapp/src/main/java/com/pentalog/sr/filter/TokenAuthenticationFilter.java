package com.pentalog.sr.filter;
...
import com.pentalog.sr.security.NoOpAuthenticationManager;
import com.pentalog.sr.security.TokenSimpleUrlAuthenticationSuccessHandler;
...
public class TokenAuthenticationFilter extends
	...
	private AbstractAuthenticationToken authUserByToken(String token) {
		...
		AuthenticationUserDetails userDetails = new AuthenticationUserDetails(
				...
				authoritiesService.getAuthorityByUsername(username));
		if (!TokenUtils.validateToken(token, userDetails)) {
			return null;
		}

		List<GrantedAuthority> authorities = new ArrayList<GrantedAuthority>();
		authorities.add(authoritiesService.getAuthorityByUsername(username).getAuthority());
