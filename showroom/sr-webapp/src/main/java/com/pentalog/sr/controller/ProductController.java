...
public class ProductController {
	...
	/**
	 * The product description service - contains methods to fetch data regarding product description from the DAO layer
	 */
	@Autowired
	...
	/**
	 * Web service that retrieves the product description from the database by product id, using the service provided by the productDescription 
	 * service.
	 * Secured with spring security, accessed only by users with admin or operator privilege.
	 * @param id -	availabilityStock id
	 * @return
	...
	public @ResponseBody ProductDescription getProductDescription(@PathVariable int id) {
		return productDescriptionService.getProductDescription(id);
	}
	
	/**
	 * Web service that persists the product description in the database, using the service provided by the productDescription service.
	 * Secured with spring security, accessed only by users with admin privilege.
	 * @param productDescription -	The product description that will be saved in the database
	 * @return
	 */
	@Secured("ROLE_ADMIN")
	@RequestMapping(value = "/description",method = RequestMethod.PUT)
	public @ResponseBody ProductDescription addProductDescription(@RequestBody ProductDescription productDescription) {
		return productDescriptionService.saveProductDescription(productDescription);
	}
	
	@Secured({"ROLE_ADMIN", "ROLE_OPERATOR"})
	@RequestMapping(value = "/description/sync", method = RequestMethod.POST)
	public @ResponseBody void syncProductDescription(@RequestBody List<ProductDescription> productsDescriptions) {
		productService.syncProductsDescription(productsDescriptions);
	}
	
	/**
	 * Web service that updates the product description in the database, using the service provided by the productDescription service.
	 * Secured with spring security, accessed only by users with admin privilege.
	 * @param productDescription -	The product description that will be updated in the database
	 * @return
	 */
	@Secured("ROLE_ADMIN")
	@RequestMapping(value = "/description",method = RequestMethod.POST)
	public @ResponseBody ProductDescription updateProductFeature(@RequestBody ProductDescription productDescription) {
		return productDescriptionService.saveProductDescription(productDescription);
	}
